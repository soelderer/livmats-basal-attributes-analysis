...
CAMaggregated_clean <- aggregate_CAMs(dat_merged = CAMfiles_clean[[3]], dat_nodes = CAMfiles_clean[[1]],
                                      ids_CAMs = sel_ids)
...
if (length(vec_delete) > 1) {
  if (all(questionnaire_clean$PROLIFIC_PID == networkIndicators_clean$participantCAM) &
    ...
    # export to MATLAB for use with GenLouvain
    writeMat(con="CAMaggregated_adj_matrices.mat", multigraph_adj_matrices_list = CAMaggregated_clean[[5]])
