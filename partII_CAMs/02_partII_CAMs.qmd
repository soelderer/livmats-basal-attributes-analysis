...
multilayer_extraction_results <- readRDS(file = "outputs/multilayer_extraction_results.Rda")
...
hist1_names_mean <- CAMwordlist[order(CAMwordlist$mean_valence, decreasing = TRUE), "Words"][1:16]
hist2_names_mean <- CAMwordlist[order(CAMwordlist$mean_valence, decreasing = TRUE), "Words"][17:33]

valenceData %>% filter(text %in% hist1_names_mean) %>%
  ggplot(aes(x = value)) + geom_histogram() + facet_wrap(~ factor(text, levels = hist1_names_mean), ncol = 4, nrow = 4) +
  ggtitle("Sorted by avg. valence")
valenceData %>% filter(text %in% hist2_names_mean) %>%
  ggplot(aes(x = value)) + geom_histogram() + facet_wrap(~ factor(text, levels = hist2_names_mean), ncol = 4, nrow = 5) +
  ggtitle("Sorted by avg. valence")


# histograms sorted by MEAN DEGREE
hist1_names_degree <- CAMwordlist[order(CAMwordlist$mean_degree, decreasing = TRUE), "Words"][1:16]
hist2_names_degree <- CAMwordlist[order(CAMwordlist$mean_degree, decreasing = TRUE), "Words"][17:33]

valenceData %>% filter(text %in% hist1_names_degree) %>%
  ggplot(aes(x = value)) + geom_histogram() + facet_wrap(~ factor(text, levels = hist1_names_degree), ncol = 4, nrow = 4) +
  ggtitle("Sorted by avg. degree")
valenceData %>% filter(text %in% hist2_names_degree) %>%
  ggplot(aes(x = value)) + geom_histogram() + facet_wrap(~ factor(text, levels = hist2_names_degree), ncol = 4, nrow = 5) +
  ggtitle("Sorted by avg. degree")


# histograms sorted by MEAN TRANSITIVITY
hist1_names_transitivity <- CAMwordlist[order(CAMwordlist$mean_transitivity, decreasing = TRUE), "Words"][1:16]
hist2_names_transitivity <- CAMwordlist[order(CAMwordlist$mean_transitivity, decreasing = TRUE), "Words"][17:33]

valenceData %>% filter(text %in% hist1_names_transitivity) %>%
  ggplot(aes(x = value)) + geom_histogram() + facet_wrap(~ factor(text, levels = hist1_names_transitivity), ncol = 4, nrow = 4) +
  ggtitle("Sorted by avg. transitivity")
valenceData %>% filter(text %in% hist2_names_transitivity) %>%
  ggplot(aes(x = value)) + geom_histogram() + facet_wrap(~ factor(text, levels = hist2_names_transitivity), ncol = 4, nrow = 5) +
  ggtitle("Sorted by avg. transitivity")
```
...
Re-import the results from GenLouvain as processed by MATLAB.

```{r}
genlouvain <- readMat("outputs/genlouvain_results.mat", drop = "singletonLists")
genlouvain <- unlist(genlouvain, recursive = FALSE)

genlouvain_results <- list()

for(i in 1:length(genlouvain)) {
  tmp_result <- unlist(unlist(genlouvain[[i]], recursive = FALSE)[[1]], recursive = FALSE)
  tmp_result <- matrix(data = tmp_result, nrow = nrow(CAMwordlist), ncol = length(CAMaggregated[[5]]))
  rownames(tmp_result) <- rownames(CAMaggregated[[5]][[1]])
  colnames(tmp_result) <- names(CAMaggregated[[5]])
  
  # named list with parameter 'omega' as name and attribute
  attributes(tmp_result)$omega <- unlist(unlist(genlouvain[[i]], recursive = FALSE)[[4]], recursive = FALSE)
  genlouvain_results[[paste0("omega", unlist(unlist(genlouvain[[i]], recursive = FALSE)[[4]], recursive = FALSE))]] <- 
    tmp_result
}
```
...
```{r}
for(i in 1:length(genlouvain_results)) {
  print(paste("Found", length(unique(as.vector(genlouvain_results[[i]]))), "communities with ",
              names(genlouvain_results)[i]))
}
```
...
genlouvain_communities <- data.frame(omega = seq(0.1, 1, 0.1), community1 = NA, community2 = NA, community3 = NA,
                                     community4 = NA, community5 = NA)

# omega = 0 with no coupling of the slices is not interesting, there are >900 communities.
# therefore we start at index 2 (omega = 0.1)
genlouvain_results <- genlouvain_results[-1]

for(i in 1:length(genlouvain_results)) {
  for(j in 1:max(unlist(genlouvain_results))) {
    genlouvain_communities[i, j+1] <- paste(rownames(genlouvain_results[[i]])[genlouvain_results[[i]][,1] == j], collapse=", ")
  }
}

DT::datatable(genlouvain_communities, options = list(pageLength = 5)) 
